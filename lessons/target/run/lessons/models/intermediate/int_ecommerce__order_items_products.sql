    ...
    create or replace table `datatalks-de-409612`.`dbt_dev`.`int_ecommerce__order_items_products`
	...
	FROM `datatalks-de-409612`.`dbt_dev`.`stg_ecommerce__products` --Refrerencing other data models 
)
SELECT
...
FROM `datatalks-de-409612`.`dbt_dev`.`stg_ecommerce__order_items` AS order_items
